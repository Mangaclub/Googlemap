...
function openPicker(id, seite, _onPick, _onCancel) {
	page = seite;
	if (_onPick) {
		onPick = _onPick;
	}
...
function showClr(color){
	...
	if (page == 'circle'){
		Circle.setLineColor();
		Circle.setFillColor();
	}else if ( page ==  'rectangle'){
		Rectangle.setLineColor();
		Rectangle.setFillColor();
	}else if ( page ==  'line'){
		Line.setColor();
	}else if ( page ==  'polygon'){
...
function getPickerContent(){
	...
	content += '<table width="100%" border="0" cellpadding="0" cellspacing="1" class="color_table"><tr><td bgcolor="#CCCCCC" id="gcpicker_colorSample" width="40px" class="choosed_color_cell">&nbsp;</td><td align="center"><div id="gcpicker_colorCode">#CCCCCC</div></td><td width="60px" align="center"><input style="width:17px; height:17px" type="submit" value="" onclick="cancel()" class="default_color_btn" /></td></tr></table>';
	content += '</td></tr><tr><td>';
